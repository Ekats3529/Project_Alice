...
data = data['person']
cities = data['hard']
cities.append(data['medium'])
cities.append(data['easy'])
...
def handle_dialog(res, req):
    ...
    if sessionStorage[user_id]['first_name'] is None:
        ...
        if first_name is None:
            ...
            res['response']['text'] = f'Приятно познакомиться, {first_name.title()}. Я Алиса. Отгадаешь человека по фото?'
        ...
        if not sessionStorage[user_id]['game_started']:
            ...
            if 'да' in req['request']['nlu']['tokens']:
                ...
                if len(sessionStorage[user_id]['guessed_cities']) == 66:
                    # если все три города отгаданы, то заканчиваем игру
                    res['response']['text'] = 'Ты отгадал всех'
...
def play_game(res, req):
    ...
    if attempt == 1:
        ...
        res['response']['card']['title'] = 'Кто это?'
        res['response']['card']['image_id'] = city['image']
        res['response']['text'] = 'Тогда сыграем!'
    else:
        # сюда попадаем, если попытка отгадать не первая
        city = sessionStorage[user_id]['city']
        # проверяем есть ли правильный ответ в сообщение
        if get_name(req) in city['answer'].lower() or req['request']['nlu']['tokens'][0] in city['answer'].lower():
            # если да, то добавляем город к sessionStorage[user_id]['guessed_cities'] и
            # отправляем пользователя на второй круг. Обратите внимание на этот шаг на схеме.
            res['response']['text'] = 'Правильно! Это {}. Сыграем ещё?'.format(city["answer"])
            ...
            if attempt == 3:
                ...
                res['response']['text'] = 'Вы пытались. Это {}.'.format(city["answer"])
                sessionStorage[user_id]['game_started'] = False
                sessionStorage[user_id]['guessed_cities'].append(city)
                return
            else:
                # иначе показываем следующую картинку
                res['response']['text'] = 'Неправильно. Вот тебе подсказка: {}'.format(city['info'])
...
def get_name(req):
    ...
    for entity in req['request']['nlu']['entities']:
        ...
        if entity['type'] == 'YANDEX.FIO':
            ...
            first = entity['value'].get('first_name', None)
            last = entity['value'].get('last_name', None)
            patr = entity['value'].get('patronymic_name', None)
